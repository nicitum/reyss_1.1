    ...
    const handleUpdateOrder = async () => {
        ...
        try {
            ...
            const productsToUpdate = products.map(p => ({
                order_id: selectedOrderId, product_id: p.product_id, name: p.name, category: p.category, price: p.price, quantity: p.quantity
            }));
    ...
    const handleAddProductToOrder = async (productToAdd) => {
        ...
        try {
            ...
            if (orderToCheck?.loading_slip === 'Yes') {
                Toast.show({ type: 'error', text1: 'Adding Product Prohibited', text2: "Loading slip already generated." });
                return;
            }
